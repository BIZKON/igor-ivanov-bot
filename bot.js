...
// v3.1: 152-ФЗ consent flow + цветные кнопки Bot API 9.4
...
// ─── 152-ФЗ CONSENT ────────────────────────────────────────
const PRIVACY_URL = "https://igor-ivanov-consult.lovable.app/privacy";
const MODERATION_CHAT = -1003783646692;

async function checkConsent(telegramId) {
  ...
  const types = (data || []).map(c => c.consent_type);
  return { pd: types.includes("pd_processing"), marketing: types.includes("marketing"), hasAny: types.length > 0 };
}

async function recordConsent(telegramId, consentType) {
...
function consentText(name) {
  return (
    `👋 Добро пожаловать, ${name}!\n\n` +
    `Я — бот Игоря Иванова, эксперта по продажам, партнёрствам и ИИ-решениям.\n\n` +
    `Через бота вы можете:\n` +
    `📚 Получить бесплатные бизнес-книги\n` +
    `🎰 Выиграть призы в рулетке\n` +
    `🤝 Стать партнёром\n` +
    `📞 Записаться на консультацию\n\n` +
    `Для работы бота мы обрабатываем ваш Telegram ID и имя из профиля.\n\n` +
    `📄 Политика конфиденциальности:\n${PRIVACY_URL}\n\n` +
    `Оператор ПДн: ИП Иванов Игорь Вадимович\n` +
    `ИНН: 781009636071, ОГРНИП: 325784700090452\n` +
    `г. Санкт-Петербург\n\n` +
    `Нажимая «Продолжить», вы подтверждаете ознакомление с Политикой конфиденциальности и даёте согласие на обработку персональных данных. Согласие действует до его отзыва.`
  );
}

function consentKb() {
  return kb(
    [cbBtn("✅ Продолжить — даю согласие", "consent_pd_yes", "success")],
    [cbBtn("📄 Подробнее", "consent_details", "primary")]
  );
}
...
function mainMenu() {
  return kb(
    ...
    [cbBtn("⚙️ Управление данными", "withdraw_consent")]
  );
}
...
async function welcome(ctx) {
  ...
  // 152-ФЗ: проверить согласие ПЕРЕД показом меню
  if (tgId) {
    const consent = await checkConsent(tgId);
    if (!consent.pd) {
      await ctx.reply(consentText(name), { reply_markup: consentKb() });
      return;
    }
  }
...
bot.callbackQuery(/^sub:(\d+):(.+)$/, async (ctx) => {
  ...
  const book = BOOKS[bookId] || BOOKS["partnership-strategy"];
  try { await supabase.functions.invoke("gift-book", { body: { gifter_telegram_id: gifterId, receiver_telegram_id: ctx.from.id, book_id: bookId } }); } catch (e) { console.error("Gift error:", e.message); }
...
bot.callbackQuery("about_bot", async (ctx) => {
  ...
  await ctx.reply(
    ...
    { parse_mode: "Markdown", reply_markup: kb([cbBtn("⬅️ В меню", "main_menu")]) }
  );
});
...
async function notifyGifter(gifterId, bookId, receiverName) {
  try {
    ...
    const tickets = p?.total_tickets || 0, gifts = p?.total_gifts || 0;
    const book = BOOKS[bookId] || BOOKS["partnership-strategy"];
    ...
    await bot.api.sendMessage(gifterId, text, { parse_mode: "Markdown", reply_markup: kb([cbBtn("🎁 Подарить ещё", `gift_${bookId}`, "success"), cbBtn("🏆 Топ", "leaderboard")]) });
...
// ─── 152-ФЗ: CONSENT HANDLERS ──────────────────────────────
bot.callbackQuery("consent_pd_yes", async (ctx) => {
  await ctx.answerCallbackQuery();
  await recordConsent(ctx.from.id, "pd_processing");
  await ctx.editMessageText(`✅ Спасибо! Согласие получено.\n\nХотите получать полезные материалы и новости от Игоря?`, {
    reply_markup: kb(
      [cbBtn("📩 Да, хочу получать", "consent_mkt_yes", "success")],
      [cbBtn("Нет, только основное", "consent_mkt_no")]
    ),
  });
});

bot.callbackQuery("consent_mkt_yes", async (ctx) => {
  await ctx.answerCallbackQuery();
  await recordConsent(ctx.from.id, "marketing");
  await ctx.editMessageText(
    `✨ *Отлично, ${ctx.from?.first_name || "друг"}!*\n\nЧто здесь есть:\n📚 Бесплатные бизнес-книги\n📋 Чек-лист «5 точек роста» в подарок\n🎰 Рулетка призов\n🎁 Розыгрыш ИИ-Экосистемы (230 000 ₽)\n\nЗаберите книгу + бонусы 👇`,
    { parse_mode: "Markdown", reply_markup: mainMenu() }
  );
});

bot.callbackQuery("consent_mkt_no", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(
...
bot.callbackQuery("consent_details", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(
    `📄 Подробности обработки данных\n\nКакие данные:\n• Telegram ID (числовой идентификатор)\n• Имя и фамилия из профиля Telegram\n• Реферальный код (если пришли по ссылке)\n\nДля чего:\n• Доступ к материалам и рулетке\n• Система достижений\n• Партнёрская программа\n\nЗащита:\n• Шифрование при передаче (HTTPS)\n• Row Level Security в базе данных\n\nВаши права:\n• Отозвать согласие: /withdraw_consent\n• Запросить удаление данных\n\nПолный текст: ${PRIVACY_URL}`,
    { reply_markup: kb(
      [cbBtn("✅ Продолжить — даю согласие", "consent_pd_yes", "success")],
      [cbBtn("⬅️ Назад", "consent_back")]
    )}
  );
});

bot.callbackQuery("consent_back", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(consentText(ctx.from?.first_name || "друг"), { reply_markup: consentKb() });
});

// ─── 152-ФЗ: ОТЗЫВ СОГЛАСИЯ ────────────────────────────────
bot.callbackQuery("withdraw_consent", handleWithdraw);
bot.hears("/withdraw_consent", handleWithdraw);
...
async function handleWithdraw(ctx) {
  ...
  const consent = await checkConsent(tgId);
  if (!consent.hasAny) {
    return ctx.reply("ℹ️ У вас нет активных согласий на обработку данных.", { reply_markup: kb([cbBtn("⬅️ В меню", "main_menu")]) });
  }
  await ctx.reply(
    `⚠️ Отзыв согласия на обработку данных\n\nВы уверены?\n\nЧто произойдёт:\n• Бот прекратит обработку ваших данных\n• Данные будут удалены в течение 30 дней\n• Вы не сможете пользоваться ботом до повторного согласия\n• Факт отзыва хранится 3 года (срок исковой давности)\n\nОтозвать согласие?`,
    { reply_markup: kb(
      [cbBtn("🔴 Да, отозвать", "confirm_withdraw", "danger")],
      [cbBtn("⬅️ Нет, вернуться", "main_menu")]
    )}
  );
}

bot.callbackQuery("confirm_withdraw", async (ctx) => {
  await ctx.answerCallbackQuery();
  const tgId = ctx.from.id;
  const { data: count } = await supabase.rpc("withdraw_consent", { p_telegram_id: tgId, p_consent_type: null });
  try {
    await bot.api.sendMessage(MODERATION_CHAT,
      `🔴 ОТЗЫВ СОГЛАСИЯ ПДн\n\nПользователь: ${ctx.from.first_name || ""} ${ctx.from.last_name || ""}\n@${ctx.from.username || "нет"}\nTG ID: ${tgId}\nДата: ${new Date().toLocaleString("ru-RU", { timeZone: "Europe/Moscow" })}\nСрок удаления: ${new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString("ru-RU")}`
    );
  } catch (e) { console.error("Notify admin withdraw:", e.message); }
  await ctx.editMessageText(`✅ Запрос на отзыв согласия принят.\n\nВаши данные будут удалены в течение 30 дней.\n\nЕсли передумаете — напишите /start для повторного согласия.\n\nПо вопросам: @igorivanov_top`);
});
...
const server = http.createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/") {
    ...
    return res.end(JSON.stringify({ status: "ok", bot: BOT_USERNAME, v: "3.1-152fz-consent" }));
  }
